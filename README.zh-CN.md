...
主界面展示所有已配置接口的概览。每个接口卡片显示名称、连接状态、网关 IP 和路由数量。点击卡片可展开查看具体路由。使用 **启动** / **停止** 按钮控制监控。

### 添加路由

1. 进入 **路由** 页面。
2. 输入网络接口名称（如 `ppp0`、`utun3`），点击 **添加接口**。
3. 在接口下方输入路由目标 — 域名（`github.com`）、IP 地址（`192.168.1.1`）或 CIDR 地址段（`10.0.0.0/8`） — 点击 **添加**。
4. 按需添加多条路由，然后点击 **保存并应用**。配置立即生效，无需重启。

### 连通性测试

在仪表盘中展开接口卡片，点击路由旁的 **Ping** 按钮。结果会显示延迟或"不可达"。

### 日志

**日志** 页面实时显示事件（接口上线/下线、路由添加/删除、错误）。可按日志级别筛选或关键词搜索。向上滚动会暂停自动滚动，滚动到底部恢复。
